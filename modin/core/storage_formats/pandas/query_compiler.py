            ...
            if self._modin_frame.has_materialized_columns:
                if left_on is None and right_on is None:
                    ...
                    if left_on is None or right_on is None:
                        raise MergeError(
                            "Must provide both 'left_on' and 'right_on' or only 'on', not a combination of them."
                        )
                    _left_on, _right_on = left_on, right_onbuted on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF
